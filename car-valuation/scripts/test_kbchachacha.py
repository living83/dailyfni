"""Live smoke test: K7 2014 against KB차차차."""
...
from scrapers.kbchachacha_scraper import KbChachachaScraper  # noqa: E402
from scrapers.utils import ScraperBlocked, ScraperEmpty  # noqa: E402

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s")
log = logging.getLogger("test_kb")
...
async def main() -> int:
    ...
    t0 = time.monotonic()
    try:
        async with KbChachachaScraper(headless=True) as scraper:
    ...
    hit = await get_cached(model, year, "kbchachacha")
    assert hit is not None and hit["median_price"] == result["median"]
    log.info("CACHE OK")
    return 0
