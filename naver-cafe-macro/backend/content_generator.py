...
STORY_TEMPLATES = [
    ...
    # ━━━ 스토리 1: 체험 후기형 ━━━
    {
        "intro": [
            ...
            ("가족한테 말하기도 그렇고 친구한테 손 벌리기도 민망하고", STYLE_NORMAL),
            ("그런 상황에서 제일 먼저 하게 되는 게 검색이잖아요", STYLE_NORMAL),
            ...
            ("제가 알아본 내용 중에서 진짜 핵심만 뽑았어요", STYLE_NORMAL),
        ],
        "body": [
            ("", STYLE_EMPTY),
            ...
            ("", STYLE_EMPTY),
            ("실제로 제가 처음에 연락한 곳은 사업자번호도 없었어요", STYLE_NORMAL),
            ("전화 상담할 때 이것저것 물어보니까 대답도 제대로 못 하더라구요", STYLE_NORMAL),
            ("그래서 바로 끊고 다시 {keyword} 제대로 알아봤죠", STYLE_NORMAL),
            ...
            ("공식적으로 등록된 업체인지 사업자 정보 공개 여부를 꼭 확인하시고", STYLE_NORMAL),
            ("상담할 때 수수료나 조건을 투명하게 알려주는지도 보세요", STYLE_NORMAL),
        ...
        "closing": [
            ...
            ("", STYLE_EMPTY),
            ("처음에는 막막해 보여도 하나씩 알아가다 보면 길이 보여요", STYLE_NORMAL),
            ("저도 처음엔 아무것도 몰랐는데 지금은 이렇게 후기까지 쓰고 있잖아요", STYLE_NORMAL),
            ("혹시 고민되시면 전문 상담부터 받아보세요!", STYLE_NORMAL),
            ("무료 상담이니까 부담 없이 한번 물어보시는 것도 방법이에요", STYLE_NORMAL),
    ...
    # ━━━ 스토리 2: 정보 공유형 ━━━
    {
        "intro": [
            ...
            ("심지어 같은 {keyword}인데 조건이 완전 다른 곳도 있고", STYLE_NORMAL),
            ("어떤 곳은 너무 좋은 조건이라 오히려 의심이 가더라구요", STYLE_NORMAL),
        ...
        "body": [
            ...
            ("어떤 곳은 수수료를 따로 받기도 하고", STYLE_NORMAL),
            ("어떤 곳은 중간에 숨겨진 비용이 있을 수도 있어요", STYLE_NORMAL),
            ("그래서 상담받을 때 총 비용이 얼마인지 꼭 물어보세요", STYLE_NORMAL),
            ("", STYLE_EMPTY),
            ...
            ("너무 좋은 말만 가득한 후기보다 장단점이 같이 있는 후기가 신뢰가 가요", STYLE_NORMAL),
            ("저도 후기 보면서 괜찮은 곳 3군데 정도 추렸거든요", STYLE_NORMAL),
        ...
        "closing": [
            ...
            ("", STYLE_EMPTY),
            ("한 군데만 보지 말고 최소 2~3곳은 비교해보시는 게 좋아요", STYLE_NORMAL),
            ("비교하다 보면 어디가 좋고 나쁜지 자연스럽게 감이 와요", STYLE_NORMAL),
            ("꼭 비교해보시고 신중하게 결정하세요!", STYLE_NORMAL),
            ("도움이 필요하시면 전문 상담 한번 받아보시는 것도 추천해요", STYLE_NORMAL),
            ("무료로 상담해주는 곳이 많으니까 부담 갖지 마시고요", STYLE_NORMAL),
    ...
    # ━━━ 스토리 3: 고민 해결형 ━━━
    {
        "intro": [
            ...
            ("", STYLE_EMPTY),
            ("주변 사람들한테 말하기도 힘들고 혼자 끙끙 앓았어요", STYLE_NORMAL),
            ...
            ("5분만 투자해서 읽어보시면 분명 도움이 될 거예요", STYLE_NORMAL),
        ],
        "body": [
            ("", STYLE_EMPTY),
            ...
            ("거기서 이미 개인정보도 줬는데 나중에 생각하니까 아찔하더라구요", STYLE_NORMAL),
            ...
            ("제가 여러 가지 질문을 했는데 하나도 귀찮아하지 않고 다 답해주시더라구요", STYLE_NORMAL),
            ("조건도 투명하게 알려주시고 숨기는 게 없어서 믿음이 갔어요", STYLE_NORMAL),
            ("{keyword} 알아볼 때 이런 곳을 찾으셔야 해요", STYLE_NORMAL),
            ("억지로 밀어붙이는 곳은 피하세요 제발요", STYLE_NORMAL),
            ("처음에 너무 쉽게 된다고 말하는 곳도 조심하세요", STYLE_NORMAL),
        ],
        "closing": [
            ("", STYLE_EMPTY),
            ...
            ("저도 처음엔 안 될 줄 알았는데 방법이 있었거든요", STYLE_NORMAL),
            ("포기하지 않고 알아본 게 진짜 다행이었어요", STYLE_NORMAL),
    ...
    # ━━━ 스토리 4: 비교 분석형 ━━━
    {
        ...
        "body": [
            ...
            ("", STYLE_EMPTY),
            ("정식 업체의 특징을 말씀드릴게요", STYLE_NORMAL),
            ("홈페이지에 사업자 정보가 공개되어 있어요", STYLE_NORMAL),
            ("상담 과정에서 모든 조건을 투명하게 안내해줘요", STYLE_NORMAL),
            ("무료 상담을 제공하고 결정을 강요하지 않아요", STYLE_NORMAL),
            ("", STYLE_EMPTY),
            ("반면에 중개 업체는 수수료가 추가로 붙는 경우가 많고", STYLE_NORMAL),
            ("나중에 문제가 생겨도 책임을 안 지려고 하는 경우가 있어요", STYLE_NORMAL),
            ("처음에 좋은 말만 하고 나중에 조건이 달라지는 경우도 있구요", STYLE_NORMAL),
            ("심지어 개인정보만 수집해서 여러 곳에 넘기는 데도 있어요", STYLE_NORMAL),
        ...
        "closing": [
            ...
            ("여러 군데 동시에 하면 어디서 뭘 말했는지도 헷갈려요", STYLE_NORMAL),
            ("차분하게 하나씩 진행하는 게 결국엔 가장 빠른 길이에요", STYLE_NORMAL),
    ...
    # ━━━ 스토리 5: 경험담형 ━━━
    {
        ...
        "body": [
            ...
            ("요즘 사이트 하나 만드는 건 누구나 할 수 있으니까", STYLE_NORMAL),
            ("사업자등록증을 직접 확인할 수 있는 곳이 안전해요", STYLE_NORMAL),
            ("", STYLE_EMPTY),
            ("둘째 상담 시 선입금을 요구하면 무조건 피하세요!", STYLE_NORMAL),
            ("정상적인 곳은 절대 먼저 돈을 요구하지 않아요", STYLE_NORMAL),
            ("이건 진짜 백 프로 사기라고 보시면 돼요", STYLE_NORMAL),
            ("", STYLE_EMPTY),
            ...
            ("비교해보니까 조건 차이가 꽤 나더라구요", STYLE_NORMAL),
        ],
        "closing": [
            ("", STYLE_EMPTY),
            ...
            ("물어볼 거 다 물어보고 맘에 안 들면 안 하면 되는 거예요", STYLE_NORMAL),
            ("아무것도 안 하고 고민만 하는 게 제일 안 좋아요", STYLE_NORMAL),
